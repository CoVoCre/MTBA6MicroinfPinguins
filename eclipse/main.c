...
int main(void)
{
	...
	//start motor controller, but do not move yet.
	//updateAngle is the callback to give new destination angles to controller
    ...
    while (1) {
		...
		//get how many sources were detected, in order to present them to user for choosing
    		...
    		if(nb_sources==ERROR_AUDIO){
        		 ...
        		 */
        		while(1){} //TODOPING stop everything else, create function for this
    		}
		...
		{	//enter a new block (scope) to discard endTextReadPointer right after
        	...
        	robotMoving=true;
        	travCtrl_startStop(robotMoving); //start movements of the robot as angle will be updated asap //TODOPING should we do this later ?
